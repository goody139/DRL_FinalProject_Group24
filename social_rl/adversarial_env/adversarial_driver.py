...
class AdversarialDriver(object):
    ...
    def run(self, random_episodes=False, collect_video=False):
        ...
        if random_episodes:
            ...
            if self.combined_population:
                agent_r_max, train_idxs = self.combined_population_adversarial_episode(collect_video)
            else:
                agent_r_max, train_idxs = self.adversarial_episode(collect_video)
        else:
    ...
    def adversarial_episode(self, collect_video=False):
        ...
        agent_r_avg, agent_r_max, agent_idx, videos = self.run_agent(
            self.env, self.agent, self.env.reset_agent, self.env.step, collect_video=collect_video)
        ...
        if self.adversary_agent:
            adv_agent_r_avg, adv_agent_r_max, antag_idx, _ = self.run_agent(
        ...
        if self.adversary_agent:
            ...
            env_reward += self.budget_objective(
        ...
        # Store video
        name = str(time.time())+'.gif'
        # get tod to save videos in
        save_dir = self.root_dir if self.root_dir is not None else os.getcwd()
        save_dir += "/vids"
        os.makedirs(save_dir, exist_ok=True)
        for i, video in enumerate(videos):
            print("supercalifragilisticexpialidocious")
            imageio.mimwrite(save_dir+"/"+str(i)+'__'+str(time.time())+'.mov', video)
    ...
    def combined_population_adversarial_episode(self, collect_video=False):
        """Episode in which adversary constructs environment and agents play it."""
        # Build environment with adversary.
        ...
        adv_r = regret + self.budget_objective(
    ...
    def log_environment_metrics(self, agent_r_max):
        ...
        tiles_found = self.env.get_num_found_tiles()
        tiles_to_map_completion = self.env.get_num_tiles_to_map_completion()
        findable_tiles = self.env.get_findable_tiles()
        for i, m in enumerate([num_blocks,
                               deliberate_placement, env_episodes,
                               tiles_found, tiles_to_map_completion, findable_tiles]):
        ...
        if self.debug:
            ...
            logging.info('Tiles found: %f',
                         tf.reduce_mean(tiles_found).numpy())
            logging.info('Tiles to map completion: %f',
                         tf.reduce_mean(tiles_to_map_completion).numpy())
            logging.info('Findable tiles: %f',
                         tf.reduce_mean(findable_tiles).numpy())
    ...
    def run_agent(self, env, agent_list, reset_func, step_func, agent_idx=None, collect_video=False):
    ...
    """
        ...
        while num_steps < agent.max_steps:
            ...
            next_time_step, collected_images = step_func((action_step.action, collect_video))
            if collected_images is not None and not collected_images.dtype == tf.dtypes.string:
                for i, vid in enumerate(collected_images):

                    if not isinstance(vid, str) and vid.shape[2] == 3:
                        if len(videos_list) <= i:
                            # create new list
                            v = []
                            v.append(vid)
                            videos_list.append(v)
                        else:
                            videos_list[i].append(vid)
        ...
        return avg_reward, max_reward, agent_idx, videos_list

    def budget_objective(self, antag_r_max, env_idx):
        ...
        # Number of blocks gives a higher reward if the antagonist scored higher
        block_budget_reward = antag_r_max * weighted_budget
