...
class AdversarialGymWrapper(gym_wrapper.GymWrapper):
  ...
  """
    ...
    def _step(self, action, collect_video=False):
        ...
        if self._done:
            return ts_lib.termination(observation, reward, outer_dims=outer_dims), images
        else:
            return ts_lib.transition(observation, reward, self._discount,
                                     outer_dims=outer_dims), images

    def step_adversary(self, action, collect_video=False):
        action = action.item() if self._action_is_discrete else action
        ...
        if self._done:
            return ts_lib.termination(observation, reward, outer_dims=outer_dims), None
        else:
            return ts_lib.transition(observation, reward, self._discount,
                                     outer_dims=outer_dims), None
...
class AdversarialTFPyEnvironment(tf_py_environment.TFPyEnvironment):
  ...
  """
    ...
    def _step(self, actions):
        ...
        def _step_py(*flattened_actions, collect_video=False):
            with tf_py_environment._check_not_called_concurrently(self._lock):  # pylint:disable=protected-access
                packed = tf.nest.pack_sequence_as(
                    structure=self.action_spec(), flat_sequence=flattened_actions)
                out = self._env.step(packed)
                self._agent_time_step, images = out
                return tf.nest.flatten(self._agent_time_step), images

        def _isolated_step_py(*flattened_actions, collect_video=False):
            return self._execute(_step_py, *flattened_actions, collect_video=False)
        ...
        with tf.name_scope('step'):
            ...
            return self._set_names_and_shapes(
                self.time_step_spec(), self.observation_spec(),
                step_type, reward, discount, *flat_observations), images
    ...
    def step_adversary(self, actions):
        ...
        def _step_adversary_py(*flattened_actions):
            with tf_py_environment._check_not_called_concurrently(self._lock):  # pylint:disable=protected-access
                ...
                self._time_step, _ = self._env.step_adversary(packed)
                return tf.nest.flatten(self._time_step)
        ...
        with tf.name_scope('step_adversary'):
            ...
            return self._set_names_and_shapes(
                self.adversary_time_step_spec, self.adversary_observation_spec,
                step_type, reward, discount, *flat_observations), None
